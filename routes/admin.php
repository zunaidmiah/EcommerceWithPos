...
Route::group(['prefix' => 'admin', 'middleware' => ['auth', 'admin', 'prevent-back-history']], function () {
    ...
    Route::controller(OrderController::class)->group(function () {
        ...
        // delete all orders
        Route::post('/orders/destroy-all', 'destroy_all_orders')->name('orders.destroy-all');

        //orders prints out all
        Route::post('/orders/print-all/invoice', [InvoiceController::class, 'bulk_invoice_download'])->name('orders.print-invoice_all');
        Route::post('/orders/print-all/pdf', [InvoiceController::class, 'select_invoice_download'])->name('orders.print-all');
        Route::post('/orders/print-all/label', [InvoiceController::class, 'print_label'])->name('orders.label-all');
        
        Route::post('/orders/change-status', 'change_status')->name('orders.change-status');

        //Delivery Boy Assign
        Route::post('/orders/delivery-boy-assign', 'assign_delivery_boy')->name('orders.delivery-boy-assign');

        // Order bulk export
        Route::post('/order-bulk-export', 'orderBulkExport')->name('order-bulk-export');
    });
    ...
    Route::controller(CourierController::class)->group(function () {
       ...
       //Redx Api routes
        ...
        Route::post('/courier/pathao/order', 'pathao_order_sent')->name('courier.pathao');
        Route::post('/courier/pathao/order/all', 'pathao_order_sent_all')->name('courier.pathao_all');
        Route::get('/courier/pathao_city', 'pathao_get_city')->name('pathao.get_city'); // get citys  
        Route::get('/courier/pathao_zone', 'pathao_get_zone')->name('pathao.get_zone'); // get zones  
        Route::get('/courier/pathao_area', 'pathao_get_area')->name('pathao.get_area'); // get zones  
