                                                ...
                                                <div class="row gutters-5 align-items-center">
                                                    <div class="col-auto w-60px">
                                                        <div class="row no-gutters align-items-center flex-column aiz-plus-minus">
                                                            <button class="btn col-auto btn-icon btn-sm fs-15" type="button" data-type="plus" data-field="qty-{{ $key }}" @if($product->digital == 1) disabled @endif>
                                                                <i class="las la-plus"></i>
                                                            </button>
                                                            <input type="text" name="qty-{{ $key }}" id="qty-{{ $key }}" class="col border-0 text-center flex-grow-1 fs-16 input-number" placeholder="1" value="{{ $cartItem['quantity'] }}" min="{{ $product->min_qty }}" max="{{ $stock->qty }}" onchange="updateQuantity({{ $key }})">
                                                            <button class="btn col-auto btn-icon btn-sm fs-15" type="button" data-type="minus" data-field="qty-{{ $key }}" @if($product->digital == 1) disabled @endif>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            $('#product-list').on('click','.add-plus:not(.c-not-allowed)',function(){
                ...
                $.post('{{ route('seller.pos.addToCart') }}',{_token:AIZ.data.csrf, stock_id:stock_id}, function(data){
                    if(data.success == 1){
        ...
        $("#confirm-address").click(function (){
            ...
            $.ajax({
                ...
                method: "POST",
                url: "{{route('seller.pos.set-shipping-address')}}",
                data: data,
                cache: false,
                contentType: false,
                processData: false,
        ...
        function removeFromCart(id){
            $.post('{{ route('seller.pos.removeFromCart') }}', {_token:AIZ.data.csrf, id:id}, function(data){
                updateCart(data);
            });
        }

        function updateQuantity(cartId){
            $.post('{{ route('seller.pos.updateQuantity') }}',{_token:AIZ.data.csrf, cartId:cartId, quantity: $('#qty-'+cartId).val()}, function(data){
                if(data.success == 1){
                    updateCart(data.view);
                }else{
                    AIZ.plugins.notify('danger', data.message);
        ...
        function setDiscount(){
            ...
            $.post('{{ route('seller.pos.setDiscount') }}',{_token:AIZ.data.csrf, discount:discount}, function(data){
                updateCart(data);
            });
        }
        ...
        function setShipping(){
            ...
            $.post('{{ route('seller.pos.setShipping') }}',{_token:AIZ.data.csrf, shipping:shipping}, function(data){
                updateCart(data);
            });
        }
        ...
        function getShippingAddress(){
            $.post('{{ route('seller.pos.getShippingAddress') }}',{_token:AIZ.data.csrf, id:$('select[name=user_id]').val()}, function(data){
                $('#shipping_address').html(data);
            });
        }

        function updateSessionUserCartData(){
            $.post('{{ route('seller.pos.updateSessionUserCartData') }}',{_token:AIZ.data.csrf, userId:$('select[name=user_id]').val()}, function(data){
                updateCart(data);
            });
        }
        ...
        function orderConfirmation(){
            ...
            $.post('{{ route('seller.pos.getOrderSummary') }}',{_token:AIZ.data.csrf}, function(data){
        ...
        function submitOrder(payment_type){
            ...
            $.post('{{ route('seller.pos.order_place') }}',{
